...
It can run BOTH versions of the experiment we have already developed:
    ...
    2. fixed_fraction sweep
       After the free-fraction experiment, the total stellar-template weights
       are fixed in turn to f_A = 0.50, 0.60, 0.70, 0.80, and 0.90. For each
       fixed-fraction run, the injected spectrum uses that same true f_A.
...
# Run the original free-light-fraction experiment once, then sweep a set of
# fixed true/fitted light fractions. Each fixed-fraction run injects the same
# f_A that pPXF is told to hold fixed.
RUN_FREE_FRACTION = True
FIXED_FRACTION_A_VALUES = np.array([0.50, 0.60, 0.70, 0.80, 0.90], dtype=float)
...
# Active fixed-fraction diagnostic value. This is overwritten before each
# fixed-fraction run in the 0.50--0.90 sweep.
...
def two_component_cell_fit_fixed_fraction(
    ...
    template-weight fraction of component 0 is fixed to the currently active
    FIXED_FRACTION_A.
...
def configure_red_setup(setup_name, fraction_mode, fixed_fraction_A=None):
    """Overwrite only the globals that differ from the blue refined run."""
    global FIXED_FRACTION_A

    cfg = RED_CONFIGS[setup_name]
    slicer_cfg = SLICER_CONFIGS[SLICER]

    # The free-fraction experiment is left unchanged and uses TARGET_FRAC_A
    # from target_config.txt. In the fixed-fraction sweep, the injected true
    # light fraction and the pPXF fixed fraction are deliberately identical.
    if fraction_mode == "free_fraction":
        injected_frac_A = float(TARGET_FRAC_A)
        run_fraction_label = "free_fraction"
    elif fraction_mode == "fixed_fraction":
        if fixed_fraction_A is None:
            raise ValueError(
                "fixed_fraction_A must be supplied for fixed_fraction mode."
            )
        fixed_fraction_A = float(fixed_fraction_A)
        if not (0.0 < fixed_fraction_A < 1.0):
            raise ValueError("fixed_fraction_A must lie between 0 and 1.")

        FIXED_FRACTION_A = fixed_fraction_A
        injected_frac_A = fixed_fraction_A
        run_fraction_label = "fixed_fA_{}".format(
            "{:.2f}".format(fixed_fraction_A).replace(".", "p")
        )
    else:
        raise ValueError("Unknown fraction mode: {}".format(fraction_mode))

    # Update the injected target fraction for THIS run. This is what makes,
    # e.g., fixed_fA_0p70 a genuine 70/30 mock rather than a 50/50 mock fitted
    # with an incorrect 70/30 constraint.
    base.TARGETS[TARGET_NAME]["frac_A"] = float(injected_frac_A)
    base.TARGET_FRAC_A = float(injected_frac_A)
    ...
    # Keep the original >=15% recovered-light requirement for the free fit.
    # In a fixed-fraction experiment the light ratio is imposed by construction,
    # so that criterion is not informative. In particular, leaving it at 0.15
    # would force every true 90/10 run to fail automatically even if both
    # velocities and dispersions were recovered perfectly.
    if fraction_mode == "free_fraction":
        base.MIN_COMPONENT_LIGHT = 0.15
    else:
        base.MIN_COMPONENT_LIGHT = 0.0

    base.MIN_RECOVERED_SEPARATION = 50.0
    base.NULL_PERCENTILE = 95.0

    # Select free-fraction or fixed-fraction fitting.
    if fraction_mode == "free_fraction":
    ...
    # Separate output directory for every free/fixed-fraction experiment.
    base.OUTPUT_DIR = (
        OUTPUT_PARENT /
        "{}_{}".format(setup_name, run_fraction_label)
    )
    ...
    return {
        ...
        "fraction_mode": run_fraction_label,
        "injected_frac_A": float(injected_frac_A),
        "fixed_fraction_A": (
            float(FIXED_FRACTION_A)
            if fraction_mode == "fixed_fraction"
            else np.nan
        ),
        "R": resolving_power,
...
def run_one(setup_name, fraction_mode, fixed_fraction_A=None):
    meta = configure_red_setup(
        setup_name,
        fraction_mode,
        fixed_fraction_A=fixed_fraction_A,
    )
    ...
    print("\n" + "#" * 78)
    print("KCRM RED TEST: {} | {}".format(setup_name, meta["fraction_mode"]))
    print("#" * 78)
    ...
    print("Injected f_A:               {:.2f}".format(meta["injected_frac_A"]))
    if np.isfinite(meta["fixed_fraction_A"]):
        print("Fixed fitted f_A:           {:.2f}".format(meta["fixed_fraction_A"]))
    else:
        print("Fixed fitted f_A:           free")
    ...
    recovery.insert(1, "fraction_mode", meta["fraction_mode"])
...
def main():
    ...
    for setup_name in RED_SETUPS_TO_RUN:
        ...
        # First run the existing free-fraction experiment unchanged.
        if RUN_FREE_FRACTION:
            meta, recovery, sigma = run_one(
                setup_name,
                "free_fraction",
            )
        ...
        # Then run the fixed-fraction sweep from f_A = 0.50 to 0.90. In each
        # case the injected true fraction and the fitted fixed fraction match.
        for fixed_fraction_A in FIXED_FRACTION_A_VALUES:
            meta, recovery, sigma = run_one(
                setup_name,
                "fixed_fraction",
                fixed_fraction_A=float(fixed_fraction_A),
