...
CRD_ROOT = Path("/Users/maxpiper/Desktop/CRD_Thesis")
CRD_DECOMP_ROOT = Path("/Users/maxpiper/Desktop/CRD_Decomposition")
SPS_FILE = None
...
# =============================================================================
# ROBUST MEAN FOR BOOLEAN / NaN SUMMARY FLAGS
# =============================================================================

def safe_nanmean(values):
    """
    Robust mean for columns containing booleans and/or NaNs.

    This is especially important for metal_order_correct.  When the two
    injected metallicities are identical, metal_order_correct is intentionally
    NaN for every Monte Carlo realization.  Older NumPy versions can fail on
    np.nanmean() when pandas stores such a column with object dtype.

    Returns NaN when there are no finite/meaningful values.
    """
    s = pd.Series(values).copy()

    # Be robust both to the in-memory DataFrame (True/False objects) and to a
    # CSV re-read where booleans may occasionally appear as strings.
    s = s.replace({
        "True": 1.0,
        "False": 0.0,
        "true": 1.0,
    ...
    for key, sub in trials.groupby(
        ...
        row = {
            ...
            "fraction_recovery_fraction":
                safe_nanmean(sub["fraction_pass"]),
            ...
            "age_A_recovery_fraction":
                safe_nanmean(sub["age_A_pass"]),

            "age_B_recovery_fraction":
                safe_nanmean(sub["age_B_pass"]),

            "age_order_fraction":
                safe_nanmean(sub["age_order_correct"]),
            ...
            "metal_A_recovery_fraction":
                safe_nanmean(sub["metal_A_pass"]),

            "metal_B_recovery_fraction":
                safe_nanmean(sub["metal_B_pass"]),

            "metal_order_fraction":
                safe_nanmean(sub["metal_order_correct"]),
            ...
            "joint_population_recovery_fraction":
                safe_nanmean(sub["joint_population_pass"]),
